...
export const getProducts=async(_,res)=>res.json(await Product.find().sort({createdAt:-1}));
...
export const deleteProduct=async(req,res)=>{await Product.findByIdAndDelete(req.params.id);res.json({message:'Deleted'});};
