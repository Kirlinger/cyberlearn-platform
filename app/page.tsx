...
import { useLanguage, useSearch, useActiveSection } from '@/app/providers';
...
function SectionRevealHome({ children, className = '' }: { children: React.ReactNode; className?: string }) {
...
export default function HomePage() {
  ...
  const filteredLessons = lessons.filter((l) => {
    ...
    return (
      ...
      l.definitionFr.toLowerCase().includes(q)
    );
  });
  ...
  const bySection = filteredLessons.reduce<Record<string, number>>((acc, l) => {
    ...
    <div className="space-y-10">
      ...
      <SectionRevealHome>
        <div className="flex items-center justify-between">
          ...
          <Link href="/courses" className="text-sm font-medium text-brand-500 hover:text-brand-600">
            {ui.heroCtaSecondary} →
          </Link>
        </div>
